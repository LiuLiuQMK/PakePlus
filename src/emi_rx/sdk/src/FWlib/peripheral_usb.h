...
// =============================================================================
/// Device descriptor structure 设备描述符
// =============================================================================
...
// =============================================================================
/// Config descriptor structure	配置描述符
// =============================================================================
typedef struct __attribute__((packed))
{
    uint8_t  size;        		//配置描述符长度 0x09
    uint8_t  type;        		//配置描述符类型 0x02
    uint16_t totalLength; 		//配置描述符总长度
    uint8_t  nbInterface; 		//有几个接口
    uint8_t  configIndex; 		//配置描述符编号
    uint8_t  iDescription;		//描述符字符串
    uint8_t  attrib;      		//bit:7 = 1, bit:6 = 0;表述USB总线供电 bit:5 是否支持远程唤醒。
    uint8_t  maxPower;    		//接口做大工作电流
} USB_CONFIG_DESCRIPTOR_REAL_T;

// =============================================================================
/// Interface descriptor structure 接口描述符
// =============================================================================
typedef struct __attribute__((packed))
{
    uint8_t  size;            		//接口描述符长度 0x09
    uint8_t  type;            		//接口描述符类型 0X04
    uint8_t  interfaceIndex;  		//接口编号，从0开始
    uint8_t  alternateSetting;		//备用编号、一般不用
    uint8_t  nbEp;            		//这个接口用了几个端点
    uint8_t  usbClass;        		//后面三个字节都是USB协议版本
    uint8_t  usbSubClass;     		//是否支持BIOS
    uint8_t  usbProto;        		//鼠标协议选0X02 键盘协议选0X01
    uint8_t  iDescription;    		//接口描述符字符串
...
typedef struct __attribute__((packed))
{
    uint8_t  size;      		//端点描述符长度 0x07
    uint8_t  type;      		//端点描述符类型 0X05
    uint8_t  ep;        		//bit8 1:IN 0:OUT 使用哪个USB端点
    uint8_t  attributes;		//传输方式 控制传输、中断传输、批量传输
    uint16_t mps;       		//传输一包的长度
    uint8_t  interval;  		//传输间隔
