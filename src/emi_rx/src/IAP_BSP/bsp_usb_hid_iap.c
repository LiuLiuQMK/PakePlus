...
static USB_HID_OperateSta_t bsp_usb_hid_keyboard_basic_report_start(void)
{
    ...
    USB_SendData(ConfigDescriptor.ep_kb[EP_KB_IDX_GET(EP_KB_IN)].ep, KeybReport.Key_Byte_Table, KEY_TABLE_LEN, 0);
                    ...
                    if(event->data.ep == EP_CTL_OUT){   //OUT端点5
                        ...
                        switch(DataRecvBuf[0]){
                            case CTL_REPORT_ID:{    //IAP升级数据
                                /*获取数据*/
                                /*IAP 驱动下发63 字节 数据，repro ID: 0x3F*/
...
void bsp_usb_device_remote_wakeup(void)
{
    ...
    // TODO delay 10ms
    // TODO USB_DeviceSetRemoteWakeupBit(U_FALSE);
}
